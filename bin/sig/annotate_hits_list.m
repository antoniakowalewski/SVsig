...
if isempty(TbyGene_Table) || ...
   (istable(TbyGene_Table) && height(TbyGene_Table) == 0)
    error('SVsig:NoSignificantHits', ...
    ['No significant genomic interactions were identified for annotation. ' ...
     'This typically means that no bin pairs passed the significance threshold ' ...
     'after filtering. This may occur when the cohort contains too few structural ' ...
     'variants, filtering is overly stringent, or there are no recurrent breakpoint ' ...
     'clusters. Consider relaxing the significance threshold or filtering criteria, ' ...
     'increasing the cohort size, or verifying that the input SV calls are valid.']);
end
